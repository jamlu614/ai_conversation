...
#define MIN_INTRA_TIME 100 // 100ms
...
long get_time(void)
{
	...
	if (NULL != pTempTm) {
		// snprintf(timestr, 199, "%02d:%02d:%02d.%03ld", pTempTm->tm_hour, pTempTm->tm_min,
		// 		 pTempTm->tm_sec, time.tv_usec / 1000);
       time_ms = pTempTm->tm_min * 60 * 1000 + pTempTm->tm_sec * 1000 + time.tv_usec / 1000;
	}
	return time_ms;
}
...
{
	// MIN_INTRA_TIME 100ms   if time < 100ms    ignore
	if(get_time() - last_intra_times >= MIN_INTRA_TIME){
		last_intra_times = get_time();
		enable_intra = true;
	}
	AG_LOG(INFO, "onIntraRequestReceived %lu \n",get_time());
}
