...
YuvCapturer::YuvCapturer(){};

YuvCapturer::YuvCapturer(resolution s, std::string deviceName)
...
void YuvCapturer::set_resolution(int width, int height) {
  resolution_.width = width;
  resolution_.height = height;
}
void YuvCapturer::set_device(std::string name) { deviceName_ = name; }

void YuvCapturer::set_frame_rate(int fps) {
  if (fps < 0 || fps > 100) return;
  fps_ = fps;
}

int YuvCapturer::init() {
  int ret = 0;
  ret = camera_open();
  if(ret){
...
void yuyv2yuv420(unsigned char *yuyv, unsigned char *yuv420, int width,
                 int height) {
